...
local function createButton(name, text, posY, sizeY)
    ...
    btn.Name = name
    btn.Size = UDim2.new(0, 200, 0, sizeY or 30)
...
bondFarmBtn.MouseButton1Click:Connect(function()
    if game.PlaceId == 123456789 then return end -- ❗ замените ID на свой лобби, чтобы не фармил в лобби
    ...
    if bondFarmEnabled then
        ...
        spawn(function()
            while bondFarmEnabled do
                ...
                if #bonds == 0 then
                    bondFarmEnabled = false
                    bondFarmBtn.Text = "Farm Bonds: OFF"
                    blackScreen:Destroy()
                    break
                else
